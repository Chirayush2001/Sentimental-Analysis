...
Original file is located at
    https://colab.research.google.com/drive/15gXrCzB16z6yxTvnDmNIqMim6KIKVSEc

Obejctive
To extract data from the url and save the extracted file in an outputfile and then perform textual analysis on the text file.
Parameters like Positive score,negative score etc
"""
...
"""The above code was giving an error at first 
...
# Here I am first converting all the stopwords text files to csv files and then the csv files to arrays
#Then I am creating a new text file naming it as Stopwords and then appending the data from all the arrays. At last the Stopwords text file has all the stopwords
...
import pandas 
StopWordsFile = pandas.read_csv('/content/StopWords(1-7).txt',sep='delimiter', encoding = 'ISO-8859-1')
...
import string
Fl0 = pd.read_csv('/content/File113.txt',sep='delimiter',encoding = 'ISO-8859-1')
FL0 = Fl0.to_numpy()
...
with open('People.csv', 'a', newline='') as csv_file:
  ...
  csv_writer.writerow([arr[113][1],positive_score, negative_score , polarity_score, subjectivity_score, avg_sentence_length, Fog_Index, avg_words_per_sentence, WordCount, syllable_count, count_personal_pronouns(str(Final0)), sum_of_wordcount])